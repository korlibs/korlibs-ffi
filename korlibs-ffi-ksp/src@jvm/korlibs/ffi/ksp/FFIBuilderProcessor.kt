                    ...
                    it.bufferedWriter().use {
                        ...
                        when {
                            isJvm -> {
                                ...
                                it.appendLine("  init {")
                                it.appendLine("    val options = mapOf<String?, Any?>(")
                                it.appendLine("      com.sun.jna.Library.OPTION_CLASSLOADER to this::class.java.classLoader,")
                                it.appendLine("      com.sun.jna.Library.OPTION_OPEN_FLAGS to 1,")
                                it.appendLine("      com.sun.jna.Library.OPTION_SYMBOL_PROVIDER to com.sun.jna.SymbolProvider { handle, name, parent -> parent.getSymbolAddress(handle, name, null) }")
                                it.appendLine("    )")
                                it.appendLine("    com.sun.jna.Native.register(when {")
                                it.appendLine("      com.sun.jna.Platform.isWindows() -> com.sun.jna.NativeLibrary.getInstance(\"$libraryNameWin\", options)")
                                it.appendLine("      com.sun.jna.Platform.isMac() -> com.sun.jna.NativeLibrary.getInstance(\"$libraryNameMac\", options)")
                                it.appendLine("      else -> com.sun.jna.NativeLibrary.getInstance(\"$libraryNameLinux\", options)")
                                it.appendLine("    })")
                                it.appendLine("  }")
                                it.appendLine("}")
