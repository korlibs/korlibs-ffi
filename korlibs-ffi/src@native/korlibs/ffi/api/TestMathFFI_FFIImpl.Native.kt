...
private object __TestMathFFI_FFIImpl {
    val __LIB__ = FFIDLOpenPlatform(common = "libm.so.6", linux = "libm.so.6", macos = "/usr/lib/libSystem.dylib", windows = "msvcrt")

    val cosf by lazy {
        val funcName = "cosf"
        FFIDLSym(__LIB__, funcName)?.reinterpret<CFunction<(Float) -> Float>>() ?: error("Can't find $funcName")
    }
    val malloc by lazy {
        val funcName = "malloc"
        FFIDLSym(__LIB__, funcName)?.reinterpret<CFunction<(Int) -> COpaquePointer?>>() ?: error("Can't find $funcName")
    }
    val free by lazy {
        val funcName = "free"
        FFIDLSym(__LIB__, funcName)?.reinterpret<CFunction<(COpaquePointer?) -> Unit>>() ?: error("Can't find $funcName")
    }
    val qsort by lazy {
        val funcName = "qsort"
        FFIDLSym(__LIB__, funcName)?.reinterpret<CFunction<(COpaquePointer?, Int, Int, CPointer<CFunction<(COpaquePointer?, COpaquePointer?) -> Int>>) -> Unit>>() ?: error("Can't find $funcName")
    }
    val fopen by lazy {
        val funcName = "fopen"
        FFIDLSym(__LIB__, funcName)?.reinterpret<CFunction<(CValues<ByteVar>, CValues<ByteVar>) -> COpaquePointer?>>() ?: error("Can't find $funcName")
    }
    val fclose by lazy {
        val funcName = "fclose"
        FFIDLSym(__LIB__, funcName)?.reinterpret<CFunction<(COpaquePointer?) -> Unit>>() ?: error("Can't find $funcName")
    ...
    val remove by lazy {
        val funcName = "remove"
        FFIDLSym(__LIB__, funcName)?.reinterpret<CFunction<(CValues<ByteVar>) -> Unit>>() ?: error("Can't find $funcName")
    }
}
...
internal actual class TestMathFFI_FFIImpl actual constructor() : TestMathFFI {
    ...
    actual override fun free(ptr: FFIPointer) = __TestMathFFI_FFIImpl.free(ptr.toPointer())
    actual override fun qsort(base: FFIPointer, number: Int, width: Int, compare: FFIFunctionRef<(FFIPointer, FFIPointer) -> Int>) =
    ...
    actual override fun fclose(ptr: FFIPointer) = __TestMathFFI_FFIImpl.fclose(ptr.toPointer())
    actual override fun remove(file: String) = __TestMathFFI_FFIImpl.remove(file.cstr)
